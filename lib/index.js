...
//
// Runs in the ordinary DSH Node process (NOT the dynamic sandbox). Reads its
// own bundled sprite via import.meta.url and serves two routes on the Web GUI
// origin:
//
//   GET /silverhand-pet/spritesheet.webp  -> the sprite atlas (cached)
//   GET /silverhand-pet/state             -> { state } JSON (agent state)
//
// State is derived purely from agent lifecycle EVENTS (agent/status, …), so
// the host only hard-depends on `webServer` — no `agents` service, no scoping
// surprises.
